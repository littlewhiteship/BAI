...
public class mmspider {
    ...
    private static final  String REQ_URL = "https://yimufs.tmall.com/shop/view_shop.htm?spm=a230r.7195193.1997079397.13.77536831uJZUf3";
    ...
    private void parseHtml(String content,int index) {
        if (!content.equals("")) {
            ...
            Elements item = document.select("div#tb-wrap");
            //Document document = Jsoup.parse(content);
            //jsoup选择器实现数据抽取
            //Element item = elements.get(index);
            //抽取销售量字段
            //String director = item.select("span>span.attrs").first().select("span.attrs>a").text();
            String salesvolume = item.select("ul>li>div.tm-indcon>tm-count").text();
            salesvolumeList.add(salesvolume);
    ...
    public static void main(String[] args) throws InterruptedException {
        mmspider mmspider = new mmspider();
        mmspider.getUrlList();
        mmspider.requestData();
    }
